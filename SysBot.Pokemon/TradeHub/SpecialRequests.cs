    ...
    public static class SpecialRequests
    {
        ...
        private static void LogHeldItem<T>(T pk, int heldItem, PokeRoutineExecutor<T> caller) where T : PKM, new()
        {
            ...
            if (heldItem > 0 && heldItem < allitems.Length)
            {
                ...
                caller.Log($"Ítem en mano: {itemHeld}");
            }
            else
            {
                caller.Log($"El ítem en mano estaba fuera de los límites del Array, o no se sostenía nada: {heldItem}");
            }
        }
            ...
            bool hasHomeTracker, int rewardItem, int heldItem) where T : PKM, new()
        {
            ...
            {
                detail.SendNotification(caller, "⚠️ La solicitud de naturaleza no se encontró en la base de datos.");
                return SpecialTradeType.FailReturn;
            }
        ...
        private static SpecialTradeType HandleItemRequest<T>(ref T pk, PokeRoutineExecutor<T> caller, PokeTradeDetail<T> detail) where T : PKM, new()
        {
            ...
            if (item < 0)
            {
                detail.SendNotification(caller, "⚠️ La solicitud de ítem no es válida. Verifica la ortografía y el género.");
                return SpecialTradeType.None;
            }
        ...
        private static SpecialTradeType HandleBallRequest<T>(ref T pk, PokeRoutineExecutor<T> caller, PokeTradeDetail<T> detail) where T : PKM, new()
        {
            ...
            if (ball < 0)
            {
                detail.SendNotification(caller, "⚠️ La solicitud de Poké Ball no es válida. Verifica la ortografía y el género.");
                return SpecialTradeType.None;
            }
            ...
            SaveFile sav) where T : PKM, new()
        {
            ...
            if (loaded == null)
            {
                detail.SendNotification(caller, "⚠️ ¡Esta no es una solicitud válida!");
                return SpecialTradeType.FailReturn;
            }
        ...
        private static void LegalizeIfNotLegal<T>(ref T pkm, PokeRoutineExecutor<T> caller, PokeTradeDetail<T> detail) where T : PKM, new()
        {
            ...
            if (!la.Valid)
            {
                detail.SendNotification(caller, "⚠️ ¡Esta solicitud no es legal! Intentando legalizar...");
                ...
                {
                    caller.Log($"La legalización falló: {e.Message}");
                    return;
                }
            }
            else
        ...
        private static bool HandleHomeTrackerForCloneChange<T>(bool hasHomeTracker, PokeRoutineExecutor<T> caller, PokeTradeDetail<T> detail) where T : PKM, new()
        {
            ...
            detail.SendNotification(caller, "No se pueden aplicar cambios de clon a este Pokémon. Tiene un Home Tracker y modificarlo invalidaría el rastreador. Por favor, usa un Pokémon sin Home Tracker.");
