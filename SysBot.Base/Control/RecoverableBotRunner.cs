...
public class RecoverableBotRunner<T> : BotRunner<T> where T : class, IConsoleBotConfig
{
    ...
    public void InitializeRecovery(RecoveryConfiguration config)
    {
        ...
        LogUtil.LogInfo("Servicio de recuperación de bots inicializado", "Recovery");
    }
    ...
    {
        LogUtil.LogError($"El bot {e.BotName} falló el {e.CrashTime:yyyy-MM-dd HH:mm:ss}", "Recovery");
    }

    private void OnRecoveryAttempted(object? sender, BotRecoveryEventArgs e)
    {
        LogUtil.LogInfo($"Intentando recuperación para {e.BotName} (intento {e.AttemptNumber})", "Recovery");
    }

    private void OnRecoverySucceeded(object? sender, BotRecoveryEventArgs e)
    {
        LogUtil.LogInfo($"Se ha recuperado correctamente {e.BotName} después de {e.AttemptNumber} intento(s)", "Recovery");
    }

    private void OnRecoveryFailed(object? sender, BotRecoveryEventArgs e)
    {
        LogUtil.LogError($"No se pudo recuperar {e.BotName} después de {e.AttemptNumber} intentos. Razón: {e.FailureReason}", "Recovery");
    }
