...
public sealed class BotRecoveryService<T> : IDisposable where T : class, IConsoleBotConfig
{
    ...
    private static void ValidateConfiguration(RecoveryConfiguration config)
    {
        if (config.MaxRecoveryAttempts < 1)
            throw new ArgumentException("Los intentos de recuperación máximos deben ser al menos 1", nameof(config));
        if (config.InitialRecoveryDelaySeconds < 0)
            throw new ArgumentException("Los segundos de retraso de recuperación inicial no pueden ser negativos", nameof(config));
        if (config.BackoffMultiplier < 1.0)
            throw new ArgumentException("El multiplicador de retroceso debe ser al menos 1.0", nameof(config));
    }
    ...
    private async Task MonitorBotsAsync(CancellationToken cancellationToken)
    {
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            {
                LogUtil.LogError($"Error en el monitor de recuperación de bot: {ex.Message}", "recuperación");
            }
        }
    }
            ...
            foreach (var bot in _runner.Bots)
            {
                ...
                else if (bot.IsRunning && state.ConsecutiveFailures > 0)
                {
                    ...
                    if (uptime.TotalSeconds >= _config.MinimumStableUptimeSeconds)
                    {
                        ...
                        LogUtil.LogInfo($"El bot {botName} ha estado estable por {uptime.TotalMinutes:F1} minutos. Reiniciando intentos de recuperación.", "Recovery");
                    }
                }
            }
        }
    ...
    private bool ShouldAttemptRecovery(BotSource<T> bot, BotRecoveryState state)
    {
        ...
        {
            LogUtil.LogError($"El bot {botName} ha excedido el máximo de intentos de recuperación ({_config.MaxRecoveryAttempts})", "Recovery");
            return false;
        }
        ...
        {
            LogUtil.LogError($"El bot {botName} ha fallado {state.CrashHistory.Count} veces en los últimos {_config.CrashHistoryWindowMinutes} minutos. Deshabilitando recuperación.", "Recovery");
            return false;
        }
    ...
    private async Task AttemptRecovery(BotSource<T> bot, BotRecoveryState state, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            LogUtil.LogInfo($"Intentando recuperación para el bot {botName} (intento {state.ConsecutiveFailures}/{_config.MaxRecoveryAttempts})", "Recovery");
            ...
            await Task.Run(() =>
            {
                ...
                {
                    LogUtil.LogError($"Error al iniciar el bot {botName}: {ex.Message}", "Recovery");
                    throw;
                }
            ...
            if (bot.IsRunning)
            {
                LogUtil.LogInfo($"Bot {botName} se ha recuperado correctamente", "Recovery");
        ...
        {
            LogUtil.LogError($"La recuperación falló para el bot {botName}: {ex.Message}", "Recovery");
    ...
    public void EnableRecovery()
    {
        ...
        LogUtil.LogInfo("Servicio de recuperación de bots habilitado", "Recovery");
    }
    ...
    public void DisableRecovery()
    {
        ...
        LogUtil.LogInfo("Servicio de recuperación de bots deshabilitado", "Recovery");
    }
}
