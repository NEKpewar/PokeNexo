    ...
    private async Task MonitorBotExecutionAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (!_isIntentionallyStopped && !IsStopping && !cancellationToken.IsCancellationRequested)
            {
                ...
                LogUtil.LogError($"¡El bot {Bot.Connection.Name} ha fallado inesperadamente!", "Recovery");
        ...
        {
            LogUtil.LogError($"Error en la supervisión del bot {Bot.Connection.Name}: {ex.Message}", "Recovery");
        }
    }
