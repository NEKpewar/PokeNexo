...
namespace SysBot.Pokemon.Discord
{
    ...
    public class Pokepaste : ModuleBase<SocketCommandContext>
    {
        ...
        {
            // Guard: System.Drawing usage below is Windows-only.
            if (!System.OperatingSystem.IsWindows())
            {
                await ReplyAsync("Este comando requiere Windows (System.Drawing).");
                return;
            }
            var generatingMessage = await ReplyAsync($"⏳ {Context.User.Mention} Generando y enviando tu equipo de Pokepaste. Espere por favor...");
            try
            {
                await Task.Run(async () =>
                {
                    ...
                    {
                        await ReplyAndDeleteAsync($"⚠️ {Context.User.Mention} No se encontraron conjuntos de enfrentamiento válidos en la URL de pokepaste: {pokePasteUrl}", 10, generatingMessage).ConfigureAwait(false);
                        return;
                    }
                        ...
                        foreach (var set in showdownSets)
                        {
                            try
                            {
                                ...
                                using (var cts = new CancellationTokenSource(timeout))
                                {
                                    ...
                                    {
                                        await ReplyAndDeleteAsync($"⚠️ Se produjo un tiempo de espera durante la generación {GameInfo.Strings.Species[template.Species]}. Salteando...", 10, generatingMessage).ConfigureAwait(false);
                                        continue;
                                    }
                                }
                                ...
                                {
                                    await ReplyAndDeleteAsync($"⚠️ Fallo al crear {GameInfo.Strings.Species[template.Species]}. Salteando...", 10, generatingMessage).ConfigureAwait(false);
                                    continue;
                                }
                            ...
                            {
                                var speciesName = GameInfo.GetStrings("en").Species[set.Species];
                                await ReplyAndDeleteAsync($"⚠️ Se produjo un error durante el procesamiento. {speciesName}: {ex.Message}", 10, generatingMessage).ConfigureAwait(false);
                            }
                        }
                    }
            ...
            {
                await ReplyAndDeleteAsync($"❌ Error al generar el equipo de Pokepaste: {ex.Message}", 10, generatingMessage).ConfigureAwait(false);
            }
        }
