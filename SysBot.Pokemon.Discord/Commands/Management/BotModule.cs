...
#pragma warning restore CA2021 // Do not call Enumerable.Cast<T> or Enumerable.OfType<T> with incompatible types
        ...
        private static string GetBotIPFromJsonConfig()
        {
            try
            {
                ...
                var jsonData = File.ReadAllText(PokeNexo.ConfigPath);
...
#pragma warning restore CS8602 // Dereference of a possibly null reference.
        ...
        public async Task StartBotAsync([Summary("IP address of the bot")] string? ip = null)
        {
            ...
            bot.Start();
            await ReplyAsync($"✅ Se ha iniciado el bot.").ConfigureAwait(false);
        }
        ...
        public async Task IdleBotAsync([Summary("IP address of the bot")] string? ip = null)
        {
            ...
            bot.Pause();
            await ReplyAsync($"✅ El bot se ha configurado en inactivo.").ConfigureAwait(false);
        }
        ...
        public async Task ChangeTaskAsync([Summary("Routine enum name")] PokeRoutineType task, [Summary("IP address of the bot")] string? ip = null)
        {
            ...
            await ReplyAsync($"✅ El bot ha recibido la orden de realizar **{task}** como su próxima tarea.").ConfigureAwait(false);
        }
        ...
        public async Task RestartBotAsync([Summary("IP address of the bot")] string? ip = null)
        {
            ...
            c.Reset();
            bot.Start();
            await ReplyAsync($"✅ El bot ha recibido la orden de Reiniciarse.").ConfigureAwait(false);
        }
    }
}
