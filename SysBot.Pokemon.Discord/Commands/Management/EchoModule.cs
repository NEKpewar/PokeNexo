...
#pragma warning restore CS8602 // Dereference of a possibly null reference.
            ...
            foreach (var channelEntry in Channels)
            {
                ...
                {
                    LogUtil.LogError($"⚠️ No se pudo encontrar o acceder al canal {channelId}", nameof(AnnounceAsync));
                    continue;
                }
...
#pragma warning restore CS8602 // Dereference of a possibly null reference.
        ...
        public async Task AddEchoAsync()
        {
            ...
            await ReplyAsync($"✅ ¡Se agregaron los Embed de anuncios a este canal!").ConfigureAwait(false);
        }
        ...
        private static async Task<bool> SendMessageWithRetry(ISocketMessageChannel c, string message, int maxRetries = 3)
        {
            ...
            while (retryCount < maxRetries)
            {
                ...
                {
                    LogUtil.LogError($"⚠️ No se pudo enviar el mensaje al canal '{c.Name}' (Attempt {retryCount + 1}): {ex.Message}", nameof(AddEchoChannel));
                    retryCount++;
        ...
        private static async Task<bool> RaidEmbedAsync(ISocketMessageChannel c, byte[] bytes, string fileName, EmbedBuilder embed, int maxRetries = 2)
        {
            ...
            while (retryCount < maxRetries)
            {
                ...
                {
                    LogUtil.LogError($"⚠️ No se pudo enviar el embed al canal '{c.Name}' (Attempt {retryCount + 1}): {ex.Message}", nameof(AddEchoChannel));
                    retryCount++;
